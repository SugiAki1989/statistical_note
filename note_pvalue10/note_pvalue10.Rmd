...
- $P(B|A)$: 尤度(likelohood)と呼ばれ、データが観測される確率を意味する
...
やっと尤度や事前分布のおさらいが終わったので、ここからは事後分布がどのように更新されるか確認していく。これまでの例で扱ってきた尤度の$n=10,x=3$の二項分布はベータ分布$\alpha=4,\beta=8$として表現し、
事前分布として$\alpha=3,\beta=3$のベータ分布を利用すれば、成功回数と失敗回数は各パラメタの和で計算できるため、$\alpha=7,\beta=11$のベータ分布が事後分布となる。可視化すると、少し尤度が事前分布に引き寄せられていることがわかる。事後分布を可視化したことで、パラメタ$p$がどれくらいにあるのかを確認できる。
```{r}
tibble(
  x = seq(0, 1, 0.01),
  prior = dbeta(x, 3, 3),
  likelihood = dbeta(x, 4, 8),
  posterior = dbeta(x, 7, 11)
) %>% 
...
n_trial <- 10000
set.seed(1989)
posterior <- rbeta(n = n_trial, 3 + 4, 3 + 8)
...
tibble(
  x = seq(0, 1, 0.0001),
  a = dbeta(x, prior_alpha + df_likelihood$cv[1], prior_beta + df_likelihood$not_cv[1]),
  b = dbeta(x, prior_alpha + df_likelihood$cv[2], prior_beta + df_likelihood$not_cv[2])
) %>% 
...
n_trial <- 500000
set.seed(1989)
a_sampling <- rbeta(n = n_trial, prior_alpha + df_likelihood$cv[1], prior_beta + df_likelihood$not_cv[1])
b_sampling <- rbeta(n = n_trial, prior_alpha + df_likelihood$cv[2], prior_beta + df_likelihood$not_cv[2])
