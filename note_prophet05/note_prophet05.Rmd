...
サンプルデータとして航空旅客数のこの時系列データ`AirPassengers`を利用して確認していきます。

```{r}
n <- length(AirPassengers)
df <- tibble(y = AirPassengers)
...
次は、乗法的な周期性を設定してモデルを学習させてみます。先程のモデルとの違いは、上にスパイクする箇所も、予測が上手く行っています。このように、トレンドとともに大きくなるような周期性を持つデータには、乗法的周期性を考慮したモデルで対応することでうまくいきます。
...
先程と同じく、`prophet_plot_components()`で各要素の影響を可視化しておきます。デフォルト設定のモデルと比べると、年周期の部分はパーセント表記になっています。

```{r}
prophet_plot_components(m2, fore_df2)
```

その答えはモデル式が関わっていることは確かです。乗法的な周期性を表すには、トレンドに何らかの係数をかけていくことで表現できます。例えば、下記のような感じです。
...
そうすると、各要素にトレンドが掛け合わされているため、パーセント表記にしているようです。かといって、何に対するパーセントなのかよくわかってない。調査中…。
