...
```{r}
future_df_logistic <- prophet::make_future_dataframe(m_logistic, periods = 900) %>% 
  dplyr::mutate(cap = 8)
forecast_df_logistic <- predict(object = m_logistic, df = future_df_logistic)
prophet::dyplot.prophet(x = m_logistic, fcst = forecast_df_logistic)
```
...
future_df_linear <- prophet::make_future_dataframe(m_linear, periods = 900)
forecast_df_linear <- predict(object = m_linear, df = future_df_linear)
prophet::dyplot.prophet(x = m_linear, fcst = forecast_df_linear)
```
...
future_df_linear2013 <- prophet::make_future_dataframe(m_linear2013, periods = 900)
forecast_df_linear2013 <- predict(object = m_linear2013, df = future_df_linear2013)
prophet::dyplot.prophet(x = m_linear2013, fcst = forecast_df_linear2013)
```
...
future_df_logistic2013 <- prophet::make_future_dataframe(m_logistic2013, periods = 900) %>% 
  dplyr::mutate(cap = 8.0)
forecast_df_logistic2013 <- predict(object = m_logistic2013, df = future_df_logistic2013)
prophet::dyplot.prophet(x = m_logistic2013, fcst = forecast_df_logistic2013)
```
...
```{r}
prophet::prophet(df = df, growth = "flat")$growth
```
