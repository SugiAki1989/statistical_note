...
ARIMAモデルの次数を組み合わせると非常に複雑なモデルになるので、見ただけではよくわからないので、SARIMA(2,1,1)(1,1,1)[12]を練習のために展開してみた結果がこちら。次数も少ないのでパラメタは通しで展開している。SARIMAモデルの右辺第二項で指数部分に$s$が抜けている、正しくは上記を参照。

- 【追記20250803】右辺MAの展開において、画像4行目以降、符号の正負が反対なので注意。
...
推定結果をモデルに当てはめると、下記のようなモデルを推定したことを意味している。式の2行目からは各項目の値を行ごとに記載。
