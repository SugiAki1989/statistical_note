...
さて、最初に白状しておくと、私はサッカーについて何も知らない。ワールドカップでスウェーデンがドイツに負けるのを何度か見たことはあるが、それだけだ。それでも、ここではUseR 2013データ分析コンテストの一環として提供されたリーガ・エスパニョーラのデータセットのゴール結果をモデル化する試みを紹介する。私の目標は、データセットの試合結果をモデル化するだけでなく、(a) 将来の試合で起こりうるゴール結果のオッズを計算し、(b) チームの信頼できるランキングを作成できるようにすることです。私が開発するモデルはベイズ階層モデルで、ゴール結果はポアソン分布に従って分布すると仮定します。私は、完全に指定されたベイズモデルがあるときに、Rで*簡単に*計算できるすべてのクールなことを示すことに重点を置き、モデルの比較や最も高い予測精度を持つモデルを見つけようとすることにはあまり重点を置かないつもりです（私のモデルはかなり良いと信じていますが）。同じような分析をSPSS（私の専門分野である心理学ではほとんどの人が使っている）で誰かがやってみるのを本当に見てみたい。面倒だろうけど！

まずライブラリをロードし、データを読み込み、JAGS用に前処理をする。最後の50試合は結果が未知なので、結果がわかっている試合だけを保持する新しいデータフレーム`d`を作成する。
...
d <- na.omit(laliga)
# Sampling
#d <- d %>% filter(Season %in% c("2008/09","2009/10","2010/11"))
...
これをJAGSモデルにするには、ちょっとした調整が必要である。モデルはすべての試合結果をループする必要があり、いくつかのforループが追加されます。JAGSは分散の代わりに精度（分散の逆数）で正規分布をパラメータ化するので、ハイパー事前分布を変換する必要があります。最後に、あるチームのスキルを定数に「固定」する必要があります。これらの調整を行うと、次のようなモデル記述になります：
...
rjagsと便利な`textConnection`関数を使って、Rから直接このモデルを実行できる。これは私のコンピューターで2、3分かかるが、コーヒー休憩には十分だ。
...
モデルへのこれらの変更は、残念ながらMCMCサンプラーを実行する際に多くの自己相関を導入するため、サンプル数と間引き量を増やします。私のセットアップでは、次の実行は昼休みの半分以上かかります（もう少し長く実行しても問題ないでしょう）。
