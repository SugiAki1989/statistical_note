...
```{r}
future_df <- prophet::make_future_dataframe(m, periods = 30)
...
```{r}
prophet::dyplot.prophet(x = m, fcst = forecast_df)
```
...
plts <- prophet::prophet_plot_components(m = m,
                                         fcst = forecast_df, 
                                         weekly_start = 1,
                                         yearly_start = 0,
                                         render_plot = FALSE)
...
# 学習済みのモデルを呼び出して予測
# 60日分の予測を行う
future_df2 <- prophet::make_future_dataframe(m2, periods = 60)
