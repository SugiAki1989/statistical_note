...
a <- ggplot(tmp_ts, aes(year, games)) + geom_line(stat = 'identity') + facet_wrap( ~ name, nrow = 1) + 
  theme_bw(base_size = 10) + labs(x = 'player', title = 'Number of Games')
b <- ggplot(tmp_ts, aes(year, rate_win)) + geom_line(stat = 'identity') + facet_wrap( ~ name, nrow = 1) + 
  theme_bw(base_size = 10) + labs(x = 'player', title = 'Rate of win')
...
ggplot(data = d_est, aes(x = year, y = `50%`, group = name)) +
  ...
  scale_x_continuous(breaks = seq(unique(df_matches_ts$year)[1], unique(df_matches_ts$year)[12], 2)) + 
